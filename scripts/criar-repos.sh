...
#   formulário) são ignorados. O parsing do CSV é tolerante a quebras de
#   linha CRLF e a campos entre aspas contendo vírgula (comum quando um
#   "tema" de texto livre tem vírgula e o Google Forms/Sheets aspeia o
#   campo no export).
#
...
# Evita que o gh mande respostas (ex: JSON da proteção de branch) para um
# pager interativo (less, via $PAGER/$GH_PAGER do usuário), o que pausaria
# o script em cada repositório do loop esperando 'q'.
export GH_PAGER=cat
